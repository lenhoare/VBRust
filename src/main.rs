...
fn usage() {
    eprintln!(
        "Usage:\n\
         ...
         \tvbr embed [--check] <file.rs>  expand Vinyl in `/* vbr … */` block comments in place (--check: verify only, for CI)\n\
...
/// `vbr embed <file.rs>` — Vinyl embedded in Rust. Vinyl written inside a `/* vbr …
...
/// always compiles (the Vinyl stays a comment). The Rust the fragment becomes is
/// spliced in as plain statements — call Rust functions, leave values in scope,
/// all in one flat function body.
///
/// Caveat: block comments end at the first `*/`, so embedded Vinyl can't contain a
...
fn cmd_embed(args: &[String]) {
    ...
    // stale generated region (Vinyl edited but `vbr embed` not re-run).
    let mut check = false;
    ...
    if check {
        if !errors.is_empty() {
            eprintln!("✘ {}: embedded Vinyl has errors:", input.display());
            for e in &errors {
                eprintln!("    {}", e);
            }
            exit(1);
        ...
        eprintln!("✔ {}: embedded Vinyl is up to date.", input.display());
        return;
    }
...
fn expand_embedded(text: &str) -> (String, usize, Vec<String>) {
    ...
    while i < lines.len() {
        let line = lines[i];
        // An opener is `/* vbr` (optionally with Vinyl trailing on the same line).
        ...
        // Collect the Vinyl (verbatim — no per-line prefix to strip), up to `*/`.
        ...
        if let Some(idx) = first_rest.find("*/") {
            ...
            loop {
                ...
                let l = lines[i];
                out.push(l.to_string()); // keep the Vinyl/closer line verbatim
                i += 1;
...
/// `cargo test`'s output back to the Vinyl descriptions and `.vbr` lines.
...
/// Translate libtest's plain output into Vinyl terms: one `✓ / ✗` line per test,
...
/// A starter scene: the Vinyl node class as the root, with a coloured box child so
...
/// remaining Vinyl modules keep calling it. Graduate `main.vbr` last: that
...
fn cmd_graduate(args: &[String]) {
    ...
    if file_name.ends_with(".test.vbr") {
        eprintln!(
            "✘ A `.test.vbr` module stays Vinyl — its Test blocks are the readable \
    ...
    // The other modules still written in Vinyl (tests don't count — they stay).
    ...
    if is_entry && !vbr_siblings.is_empty() {
        vbr_siblings.sort();
        eprintln!(
            "✘ The entry graduates last — these modules are still Vinyl:\n      {}\n  \
    ...
    let graduated = format!(
        "// Graduated from {} — this is the Rust your Vinyl became, now yours to keep.\n\
         // (The retired original beside it tells the build how Vinyl callers pass\n\
    ...
    if is_entry {
        ...
        eprintln!(
            "   Every module is Rust now; nothing here needs Vinyl any more. The cargo\n   \
    ...
    // honest risk is a caller that leaned on Vinyl's argument sugar toward this
    ...
    if !cargo_passes(&build, has_tests) {
        ...
        eprintln!(
            "✘ Graduation rolled back — nothing changed. (Most likely another module \
             relies on Vinyl's argument treatment when calling this one; the errors \
             above show where.)"
        );
        exit(1);
    }
    ...
    eprintln!(
        "   The Rust it generated is now the source you keep — the other modules\n   \
         ...
         it as {}.graduated: it teaches the build how Vinyl callers pass\n   \
...
fn generate_project(entry: &Path, web: bool, include_tests: bool) -> (PathBuf, Vec<FileMap>) {
    ...
    // treatment as a local one. Verbatim `.rs` modules have no Vinyl interface —
    ...
    // A graduated module (`life.rs` beside `life.vbr.graduated`) keeps its Vinyl
    // interface: the retired file records how Vinyl callers treat its arguments
    ...
    // deleted — once nothing in Vinyl calls the module (or you've adjusted the
    // callers by hand).
    ...
    if is_web_rust(&entry_compiled.rust) {
        let title = entry_compiled
            .web_title
            .clone()
            .unwrap_or_else(|| "Vinyl app".to_string());
...
//
// The transpiler records (generated-Rust line → Vinyl line) checkpoints as it
...
/// The Vinyl line a generated-Rust line came from: the last checkpoint at or
...
fn message_hint(message: &str) -> Option<&'static str> {
    if message.contains("no field `length`") || message.contains("no method named `length`") {
        return Some("A collection's length in Vinyl is `.Len()` (or `.Count()`), not `.Length`.");
    }
    None
}
    ...
    Some(match code {
        ...
        "E0425" => {
            "Rust can't find that name. Inside `Rust … End Rust` blocks and `Match` \
             patterns you're writing real Rust, so use the lowercase spelling — Vinyl's \
             `myTotal` is `mytotal` there."
        }
        "E0599" => {
            "No method with that name on this type. Method calls pass straight through \
             to Rust — check the name against Rust's String/Vec docs (Vinyl lowercases it)."
        }
        _ => return None,
    })
}
...
fn report_errors(errors: &[RustcError], locate: impl Fn(&RustcError) -> Option<(PathBuf, Vec<(usize, usize)>)>) {
    if errors.is_empty() {
        eprintln!("✘ rustc rejected the generated Rust (and produced no diagnostics Vinyl could read).");
        return;
    }
