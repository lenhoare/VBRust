# Vinyl Inline Rust — Spec

The escape hatch *and* the learning ramp. Inline Rust is Vinyl's "inline assembly":
just as 80s languages let you drop into `asm { … }` for the bits the high-level
language couldn't express, Vinyl lets you drop into a block of real Rust — and only
a plain value crosses back into Vinyl.

This is how Vinyl calls **arbitrary Rust crates** without dumping Rust's complexity
(traits, generics, lifetimes, ranges, macros) onto the Vinyl programmer: that
...
1. **Passing IN is automatic.** The block is spliced into the same function, so
   every Vinyl variable you've declared is already in scope — just write its name.
   No `pass` keyword, no re-declaration. (One catch: inside the block you use the
   *Rust* spelling — a Vinyl name is simply its lowercase self, so `myValue` is
...
This is the same mechanic Vinyl already uses for `Return x + y` (which compiles to a
...
sealed in the block. What comes out is a plain `Double`. Vinyl never has to *name*
...
result back into familiar territory. Over time the blocks grow and the Vinyl
