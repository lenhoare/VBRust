# Vinyl Example-Project Instructions

You are helping build generated example projects for a new programming language
called **Vinyl: Visual Basic Rust**.

The parent directory home/len/dev/vbrprojects/VBRust contains the Vinyl language specification. **Read it first and
...
The purpose of this exercise is to **test the existing Vinyl transpiler**, not to
improve or change the transpiler itself.

Prefer writing Vinyl code wherever possible. Avoid inline Rust unless it is
absolutely necessary. The point is to test Vinyl syntax, Vinyl semantics, and the
existing Vinyl standard library.
...
A **folder is a Vinyl project**: every `.vbr` file in it is a *module* named after
...
Each `main.vbr` should be a small, runnable Vinyl program. Keep it deterministic.

---

## Testing — use the real `Test` / `Assert` harness

Vinyl has a **built-in test harness** (`vbr test`). **Do not** hand-roll PASS/FAIL
...
- which **Vinyl language** features it tests,
...
- Use the existing Vinyl language and existing stdlib only.
- Prefer pure Vinyl over inline Rust.
