# Vinyl Godot gaps found while building Pac-Man
...
- **Load a texture/resource from code** — no `load("res://x.png")` for a
  `Texture2D`. *Workaround:* set textures in the `.tscn` (`ext_resource`); Vinyl only
...
- **Case-insensitive name collisions pass silently** — a `Row` function and a `row`
  param both became `row` in Rust (`row(row)` → error). Vinyl should *diagnose* this.
