//! In-process Vinyl compile for Watch + Rust pane (errors → jump-to-line).
...
/// 0-based inclusive Rust line range generated from a 1-based Vinyl line.
pub fn rust_span_for_vbr(
    ...
    // Nearest earlier checkpoint (same rule as rustc→Vinyl attribution).
    let (r, _) = map
        .iter()
        .rev()
        .find(|(_, v)| *v <= vbr_1)
...
/// 1-based Vinyl line for a 1-based Rust line (last checkpoint at or before it).
...
mod tests {
    ...
    fn rust_span_covers_multi_line_region() {
        // Checkpoints: rust lines 10,14,20 from Vinyl 3,3,5
