//! Emit a runnable Vinyl `Screen` from the designer tree.
//!
...
pub fn design_to_vbt(design: &Design) -> String {
    let mut out = String::new();
    out.push_str("' Vinyl Screen template — structure only. Open in tide_design.\n");
    out.push_str(&format!("Screen {}\n", design.screen_name));
    out.push_str(&format!("    Title {}\n\n", quote(&design.title)));
    emit_menu(design, &mut out);
    out.push_str("    View\n");
...
mod tests {
    ...
    fn emitted_vbr_compiles() {
        ...
        assert!(
            !compiled.has_errors,
            "emitted Vinyl failed to compile:\n{}\n--- source ---\n{src}",
