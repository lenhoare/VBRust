...
//!
//! A template is Vinyl with no extra code: one `Screen`, a `Title`, optional
...
pub fn load_template(path: &Path) -> Result<Design, String> {
    if is_vbr(path) {
        return Err(
            "That's a Vinyl program, not a template. The designer only opens .vbt \
