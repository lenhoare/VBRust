// VS Code integration for Vinyl. Two features live here:
//
...
function emitRust(sourceUri) {
  return new Promise((resolve) => {
    ...
    execFile(cmd, args, { maxBuffer: 16 * 1024 * 1024, cwd: root }, (err, stdout, stderr) => {
      ...
      const header =
        "// ── Vinyl could not produce Rust for this file ──\n" +
...
// ── Running: `.vbr` files, and `.rs` files with embedded Vinyl ────────────────
...
// [cmd, args] to invoke the Vinyl CLI: a prebuilt `vbr` binary if present (fast),
...
// One reused "Vinyl Run" terminal so successive runs don't pile up panels.
let runTerminal;
function runInTerminal(exe, args, cwd) {
  if (!runTerminal || runTerminal.exitStatus !== undefined) {
    runTerminal = vscode.window.createTerminal({ name: "Vinyl Run", cwd });
  }
...
function showErrors(title, body) {
  if (!outputChannel) outputChannel = vscode.window.createOutputChannel("Vinyl");
  outputChannel.clear();
...
// Run a `.rs` file that embeds Vinyl: expand the `/* vbr … */` block(s) with
...
async function embedAndRunRust(document) {
  ...
  // Check on click (cheap) — do nothing if there's no Vinyl to expand.
