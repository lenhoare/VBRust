# Vinyl Language Support (VS Code)

Editor assistance for `.vbr` files, powered by the Vinyl compiler itself:
...
- **this extension** — a thin VS Code client that launches the server and tells
  it which files are Vinyl. All the real work is in the server.
