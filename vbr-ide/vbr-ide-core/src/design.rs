...
//! frontend, sent over as JSON) → clean Vinyl `View` code. One direction only —
//! the design is the source, the Vinyl is a read-only artifact you paste into a
//! `Window` file, exactly as the IDE's Rust pane is a read-only artifact of your
//! Vinyl. Kept here (not the frontend) so the emitted syntax is unit-tested.
...
/// A Vinyl string literal: wrap in quotes, doubling any embedded quote (VB's
