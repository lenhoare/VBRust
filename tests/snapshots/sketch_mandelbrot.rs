...
// Mapping onto the complex plane needs a float cast; Draw doesn't insert `as`,
// so those two lines are a tiny Rust hatch. The loop and Set Pixel are Bust.
        ...
        let mut frame = iced::widget::canvas::Frame::new(renderer, bounds.size());
        {
            ...
            for y in 0..=height - 1 {
                for x in 0..=width - 1 {
                    ...
                    if n < self.maxiter {
                        let t: i32 = n * 255 / self.maxiter;
                        { let __px = (x) as i32; let __py = (y) as i32; if __px >= 0 && __py >= 0 && (__px as u32) < pw && (__py as u32) < ph { let __i = ((__py as u32 * pw + __px as u32) * 4) as usize; let __c = iced::Color::from_rgb8((t) as u8, (t / 2) as u8, (255 - t) as u8); pix[__i] = (__c.r * 255.0) as u8; pix[__i + 1] = (__c.g * 255.0) as u8; pix[__i + 2] = (__c.b * 255.0) as u8; pix[__i + 3] = 255; pix_dirty = true; } }
                    }
                }
            }
