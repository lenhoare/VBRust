// An opaque handle — `Dim words = Rust` is sugar for `Dim words As Handle`.
// Hold a Rust value Vinyl has no type for, and pass it back into later Rust
// blocks. Here: a Rust iterator, held across blocks.
