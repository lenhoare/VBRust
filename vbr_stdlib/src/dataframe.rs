//! Native dataframes for Vinyl, wrapping the `polars` crate. A `DataFrame` is a
//! table of typed columns you read, transform, and write.
//!
//! Transforms are expressed with polars **expressions** — `col`, `lit`,
//! `when/then/otherwise` — which Vinyl generates from *column formulas* (e.g.
...
// The expression builders Vinyl's generated code calls directly. `len` is the
...
/// `PolarsResult` and builder boilerplate behind clean, Vinyl-friendly methods.
#[derive(Clone)]
...
impl DataFrame {
    ...
    /// One whole-column aggregation, cast to `f64` (Vinyl's `Double`) so every
...
/// types a Vinyl `Vec<T>` can hold. Assumes the column has no null values.
