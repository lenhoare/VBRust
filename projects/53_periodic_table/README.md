...
VBR source transpiles to idiomatic Python and its output is checked
...
- **`main.vbr` + `periodic.vbr` + `periodic.test.vbr`** (project root) —
  the full CSV-parsing version: `FileSystem.ReadLines` → `Periodic.ParseRow`
...
- **`py/main.vbr`** (subfolder) — the single-file variant that `vbr py`
  can transpile. It keeps the stdlib exercise (one `FileSystem.ReadLines`
...
- `FileSystem.ReadLines` (stdlib) lowering to the Python package
