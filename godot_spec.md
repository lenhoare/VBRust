...
loads. You write Vinyl; Godot owns the scene and the game loop.

> **Godot is an optional extra, not a core Vinyl target.** Vinyl's heart is
...
> surfaces), so it feels like the rest of Vinyl, but you can ignore it entirely.
...
- **It reads like the rest of Vinyl.** A node is a familiar-looking block with
  `Export` fields and `On <Event>` bodies; inside a body you write ordinary Vinyl.
  The one Godot-ish idea is *inversion of control* — Godot calls your events; you
  don't write a `Main`.
- **Passthrough over wrapping.** Godot's API is enormous. Vinyl doesn't wrap it —
...
- **The engine owns the scene.** You build scenes, lay out nodes, and import
  assets in the Godot editor; Vinyl contributes the *behaviour scripts*. A `.tscn`
